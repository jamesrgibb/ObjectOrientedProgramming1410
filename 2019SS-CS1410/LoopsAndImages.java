...
public class LoopsAndImages {
	...
	public static void main(String[] args) {
		...
		System.out.println("The output should result in false because there are an odd amount of numbers in the string: " + hasMoreEvenThanOdd("1 2 3 4 6"));
		System.out.println("The output should result in true because there are an even amount of numbers in the string: " + hasMoreEvenThanOdd("2 3 -9 7 4 9"));
		System.out.println(makeTextTriangle(6));
		Picture picture = new Picture("Arches.jpg");
		Picture greyPicture = makeGrey(picture);
		Picture negativePicute = makeNegative(picture);
		Picture brighterPicture = makeBrighter(picture);
	}

	/**
	 * hideLetterA will change the sentence from containing the char 'a' to '*'. For example: "I ran all the way from Alabama" to "I r*n *ll the w*y from *l*b*m*"
	...
	/**
	 * hasMoreEvenThanOdd method counts the number of even and odd numbers in the string. 
	 * @param numbers - a string containing ints in a string with spaces between each one 
	...
	/**
	 * makeTextTriangle - method draws out a triangle made of stars according to the int param that is received.
	 * 
	 * @param numRows - the number of rows that the equation will execute.
	 * @return - A triangle of stars/asterisks with the number of rows that are inputed into numRows
	 */
...
/**
 * makeGrey - will turn the picture to show all grey values in the pixels. All of the color values are added up and averaged. 
 * 
...
/**
* makeNegative - takes the original picture and inverts it to give it the negative effect. It takes a 
* param of the original picture. Each color value is subtracted from 255.
...
/**
* safeColor - the method takes an int parameter for the color value and will make sure that the value is between the range of 0-255.
* 
...
/**
* makeBrighter - makes the image brighter by doubling the color value while staying within the color limits that are described.
* 
* @param picture - the picture file used 
* @return - a brighter picture with double the value of the original unless the limits are outside the range of 0-255
*/
...
for (int row = 0; row < picCopy.height(); row++) {
	for (int col = 0; col < picCopy.width(); col++) {
		Color c = picCopy.get(col, row);
		Color newColor = new Color(safeColor(c.getRed()*2) , safeColor(c.getGreen()*2), safeColor(c.getBlue()*2));
		picCopy.set(col, row, newColor);
}
}
picCopy.show();
